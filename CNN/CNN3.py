...
class LeNet5:
    def __init__(self):
        self.layers = [ # 加了padding不见得好，反而计算慢，设为0
            ConvLayer(1, 32, kernel_size=3, padding=0),  # 输入:1x8x8 -> 输出:32x6x6
            LeakyReLU(),
            AvgPoolLayer(kernel_size=2, stride=2),       # 输入:32x6x6 -> 输出:32x3x3
            ConvLayer(32, 64, kernel_size=3, padding=0),  # 输入:32x3x3 -> 输出:64x1x1
            LeakyReLU(),
            Flatten(),                                   # 输入:64x1x1 -> 输出:128
            FCLayer(64, 128), 
            LeakyReLU(),
            FCLayer(128, 64),
            LeakyReLU(),
